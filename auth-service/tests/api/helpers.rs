...
fn configure_redis() -> redis::Connection {
    let redis_hostname = SecretString::from(DEFAULT_REDIS_HOSTNAME.to_owned());
...
fn configure_postmark_email_client(base_url: String) -> PostmarkEmailClient {
    let postmark_auth_token = SecretString::from("auth_token".to_owned());

    let sender = Email::parse(SecretString::from(test::email_client::SENDER.to_owned())).unwrap();
